...
class RequestController
{
    ...
    public function create(): void
    {
        ...
        $types = $this->db->query('SELECT type_name FROM packaging_types WHERE is_active = 1 ORDER BY id DESC');
        view('requests/create', ['types' => $types]);
    }
    ...
    public function store(): void
    {
        ...
        $user = $this->auth->user();
        $serviceType = $_POST['service_type'] ?? 'standard_order';
        ...
        if (!empty($_FILES['reference_file']['name']) && is_uploaded_file($_FILES['reference_file']['tmp_name'])) {
            ...
            if (in_array($ext, ['jpg', 'jpeg', 'png', 'pdf', 'webp'], true)) {
                ...
                $target = base_path('web/uploads/' . $name);
    ...
    public function show(int $id): void
    {
        ...
        $rows = $this->db->query(
            'SELECT r.*, c.name as customer_name, c.email as customer_email, c.phone as customer_phone, u.name as assigned_name
    ...
    public function latestMessageId(int $id): void
    {
        $this->auth->requireAuth();
        $user = $this->auth->user();
        ...
        $row = $this->db->query('SELECT MAX(id) latest_id FROM request_messages WHERE request_id = ?', [$id])[0] ?? ['latest_id' => 0];
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['latest_id' => (int) ($row['latest_id'] ?? 0)]);
        exit;
    }
