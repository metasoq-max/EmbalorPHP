<div class="card">
    ...
    <p><strong>العميل:</strong> <?= e($request['customer_name']) ?> - <?= e($request['customer_phone'] ?? '-') ?></p>
    ...
    <?php if (in_array($user['role'], ['admin','worker','designer'], true) && !empty($request['customer_phone'])): ?>
        <a class="btn secondary" target="_blank" href="https://wa.me/<?= e(preg_replace('/\D+/', '', $request['customer_phone'])) ?>">تواصل واتساب مع العميل</a>
    <?php endif; ?>
</div>
...
<div class="card" id="messages-section">
    ...
    <div class="chat" id="chat-box" data-request-id="<?= (int)$request['id'] ?>" data-last-message-id="<?= !empty($messages) ? (int) end($messages)['id'] : 0 ?>">
        <?php foreach ($messages as $m): ?>
            <div class="msg">
                <strong><?= e($m['sender_name']) ?> (<?= e($m['sender_role']) ?><?= in_array($m['sender_role'], ['admin','worker','designer'], true) ? ' ✔' : '' ?>)</strong><br>
                <?= nl2br(e($m['message'])) ?>
                <div class="muted"><?= e($m['created_at']) ?></div>
            </div>
        <?php endforeach; ?>
    </div>
