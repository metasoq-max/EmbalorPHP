<div class="card">
    ...
    <p class="muted">الخدمة: العميل يرفع الديزاين ديالو، كنوفرو معاينة 3D للتأكيد، ومن بعد كنبدأو التصنيع والتسليم. أقل كمية إنتاج هي <strong>50 حبة</strong>.</p>
    <form method="post" action="<?= e(route_url('requests.create')) ?>" enctype="multipart/form-data">
        <input type="hidden" name="_token" value="<?= e(csrf_token()) ?>">
        <label>نوع الخدمة</label>
        <select name="service_type" required>
            <option value="design_to_3d">لدي تصميم وأريد تحويله إلى معاينة أومبلاج 3D</option>
